...
search_transcript() {
    ...
    if [ -n "$transcript_path" ] && [ -f "$transcript_path" ]; then
        tail -r -n 5 "$transcript_path" | grep -m 1 "$pattern" || true
    fi
}
...
# 通知を送信する関数
#
# terminal-notifier を使用して macOS 通知を送信する。
#
# 引数:
#   $1 = title    - 通知のタイトル (必須)
#   $2 = message  - 通知のメッセージ本文 (必須)
#   $3 = sound    - 通知音の名前 (必須、例: "default", "Glass")
#   $4 = subtitle - サブタイトル (オプション)
#
# 戻り値:
#   terminal-notifier の終了コード
##############################################################################
